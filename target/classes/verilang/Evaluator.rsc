...
import verilang::TypeChecker;  // custom checker (no TypePal dep); use Typecheck.rsc once TypePal is installed
import ParseTree;
import IO;
import List;
import String;
...
void run(loc file) {
  ...
  println("");
  println("--- Type check ---");
  list[str] typeErrors = verilang::TypeChecker::checkFile(file);
